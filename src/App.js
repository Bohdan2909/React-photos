...
React.useEffect(() => {
 ...
 fetch(`https://5c3755177820ff0014d92711.mockapi.io/photo_collections?page=${page}&limit=3&${category}`)
 .then((res) => res.json())
    ...
    <div className="App">
      ...
      <div className="top">
       ...
       {cats.map((obj, index) => <li onClick={() => setCategoryId(index)} className={categoryId === index ? 'active' : ''} key={obj.name}>{obj.name}</li>)}
        ...
        <input value={searchValue} onChange={onChangeInput} className="search-input" placeholder="Поиск по названию" />
      </div>
